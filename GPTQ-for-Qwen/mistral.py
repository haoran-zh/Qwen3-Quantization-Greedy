...
from transformers import MistralConfig, MistralForCausalLM, modeling_utils
...
def get_mistral(model="mistralai/Mistral-7B-Instruct-v0.2"):
    ...
    model = MistralForCausalLM.from_pretrained(model, torch_dtype=torch.bfloat16, device_map="auto")
    model.seqlen = 2048
    return model
...
def mistral_pack(model, quantizers, wbits, groupsize):
...
def load_quant(model, checkpoint, wbits, groupsize=-1, fused_mlp=True, eval=True, warmup_autotune=True):
    config = MistralConfig.from_pretrained(model)
...
def mistral_multigpu(model, gpus, gpu_dist):
...
if __name__ == '__main__':
    ...
    parser.add_argument('model', type=str, help='mistral model to load')
    ...
    if (not args.load and args.wbits < 16 and not args.nearest) or args.step:
        ...
        tick = time.time()
        quantizers = mistral_sequential(model, dataloader, DEV, args.model)
    ...
    if args.benchmark:
        ...
        if len(gpus) > 1:
            mistral_multigpu(model, gpus, gpu_dist)
        else:
            model = model.to(DEV)
        if args.benchmark:
    ...
    if args.eval:
        ...
        for dataset in datasets:
            ...
            print(dataset)
            ppl = mistral_eval(model, testloader, DEV)
    ...
    if args.eval_mae_validation:
        ...
        fp_model = get_mistral(args.model)
        fp_model.eval()
    ...
    if not args.observe and args.save_safetensors:
        mistral_pack(model, quantizers, args.wbits, args.groupsize)
